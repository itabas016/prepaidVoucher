...
namespace PayMedia.Integration.IFComponents.BBCL.PrepaidVoucher
{
    ...
    public static class ServiceUtilities
    {
        ...
        public class WCFInterceptor : IDispatchMessageInspector
        {
            /// <summary>
            ...
            /// </summary>
            public event EventHandler<WCFInterceptorEventArgs> RaiseRequestReceived;

            /// <summary>
            /// Triggered from BeforeSendReply method.
            /// </summary>
            public event EventHandler<WCFInterceptorEventArgs> RaiseSendingReply;

            protected void OnRaiseRequestReceived(string message)
            {
                EventHandler<WCFInterceptorEventArgs> handler = RaiseRequestReceived;

                if (handler != null)
                {
                    handler(this, new WCFInterceptorEventArgs(message));
                }
            }

            protected void OnRaiseSendingReply(string message)
            {
                EventHandler<WCFInterceptorEventArgs> handler = RaiseSendingReply;
        ...
        public class WCFInterceptorEventArgs : EventArgs
        {
            public WCFInterceptorEventArgs(string message)
            {
